  ...
  const endDrag = async (dropDate = null) => {
    ...
    try {
      ...
      if (dropDate && dropDate !== currentEventDate) {
        ...
        const originalStartDate = new Date(draggedEventData.value.start || draggedEventData.value.startDate)
        const originalEndDate = new Date(draggedEventData.value.end || draggedEventData.value.endDate)        // Parse della data target usando UTC
