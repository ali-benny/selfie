...
app.patch('/pomodoros/:id', async (req, res) => {
  ...
  try {
    // TODO: gestire casi della response di mongodb
    const result = await Pomodoro.findByIdAndUpdate(id, { $set: req.body })
...
app.post('/pomodoros', async (req, res) => {
  try {
    const config = new PomodoroConfig({ ...req.body.config })
    await config.save()
